...
func originResponse(
	...
	req := httptest.NewRequestWithContext(tb.Context(), http.MethodPost, "/form", nil)
	...
	if !reached && rec.Code == http.StatusTeapot {
		tb.Error("handler reached flag lost")
	}
...
func TestOriginCheck_CrossOriginRejected(t *testing.T) {
	...
	rec := originResponse(t, mw, map[string]string{"Origin": "https://evil.example.net"})
...
func TestOriginCheck_RefererFallback(t *testing.T) {
	...
	rec := originResponse(t, mw, map[string]string{"Referer": "https://app.example.com/form"})
	if rec.Code != http.StatusTeapot {
		t.Errorf("Referer fallback to allowlisted origin: status = %d, want 418", rec.Code)
	}

	rec2 := originResponse(t, mw, map[string]string{"Referer": "https://evil.example.net/form"})
...
func TestOriginCheck_CaseInsensitiveMatch(t *testing.T) {
	...
	rec := originResponse(t, mw, map[string]string{"Origin": "https://app.example.com"})
	if rec.Code != http.StatusTeapot {
...
func TestOriginCheck_AllowAllLogs(t *testing.T) {
	...
	rec := originResponse(t, mw, map[string]string{"Origin": "https://anything.example"})
	if rec.Code != http.StatusTeapot {
