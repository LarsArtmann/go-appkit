...
- [ ] **Watchlist refresh (standing):** cordis consumers count; PapDashboard v0.3.1+; nixpkgs toolchain > 1.26.7; dprint exit-14 upstream; cqrs-htmx M3/M4 — when they land the appkit v0.5.x bump + `Config.Metrics`/`Config.Version` threading, re-verify integration's "deliberately NOT setup's pin" note, refresh the AGENTS reference-consumer line, and smoke-test the metrics surface through setup once; when their default-flip (b)-(f) lands, update the reference-consumer line again.
