...
~~| 2  | Re-verify pkg.go.dev per module page once the crawler lands (expect: license UNKNOWN until #1; 404s should clear)                                                | 4      | 1      |~~ tracked in TODO_LIST P1 (pkg.go.dev verification)
~~| 3  | Notify + validate cqrs-htmx against cqrs v0.4.0 (breaking FlightRecorder change) — their side or a compatibility shim note in our README                         | 4      | 2      |~~ done — cqrs-htmx revalidated (their v4.9.x train consumes v0.3.0+; fold-in tracked consumer-side)
~~| 4  | Logging posture decision (USER GATE, data attached): default WARN vs sampling vs status quo                                                                      | 4      | 2      |~~ tracked in TODO_LIST P2 (logging posture)
~~| 5  | Install govulncheck + apidiff + benchstat into the environment (nix devShell or prebuilt) so release ritual tools stop being improvised                          | 3      | 2      |~~ NOT-DO — tools improvised per session; go-doc diff codified in the Release Ritual
~~| 6  | Add the frh v0.1.1 jsonv2-correction note to its CHANGELOG (tag annotation is immutable)                                                                         | 2      | 1      |~~ done — flightrecorderhealth CHANGELOG + docs corrected 2026-09-04
~~| 7  | Re-run the 2026-08-18 ForceFlush probe to confirm it still reproduces, then file upstream (USER GATE: issue vs PR)                                               | 3      | 2      |~~ closed — fixed upstream, verified 2026-09-15 (nothing to file)
~~| 8  | Push `httputil/docs/integrations/huma.md` (their repo) and restore the design-decisions link                                                                     | 1      | 1      |~~ done — link inlined into design-decisions (2026-09-04)
~~| 9  | Wire the 10-module sweep into a single script (`scripts/verify-all.sh`) so session-close sweeps are one command                                                  | 2      | 1      |~~ done at 9b163ce — CI per-module matrix (2026-09-07)
...
~~| 14 | benchstat before/after harness for the logging decision once implemented                                                                                         | 2      | 1      |~~ tracked in TODO_LIST P2/P3 (logging posture + benchstat)
~~| 15 | Wave-2 announcement/changelog digest for consumers (core hooks + cqrs breaking + new modules)                                                                    | 3      | 1      |~~ NOT-DO — CHANGELOGs serve as the digest
~~| 16 | pkg.go.dev badges/links in READMEs once pages render                                                                                                             | 1      | 1      |~~ tracked in TODO_LIST P1 (pkg.go.dev, gated on license)
~~| 17 | integration/ module: bump pins to wave-2 tags and exercise the new hook surface E2E                                                                              | 3      | 2      |~~ done — integration pins bumped to v0.4.0 wave tags
~~| 18 | Release-ritual checklist as a file (`docs/release-checklist.md`) instead of an AGENTS section, referenced from every module CHANGELOG template                   | 2      | 1      |~~ NOT-DO — Release Ritual stays in AGENTS.md
~~| 19 | Add `go doc` snapshot diff as a make-less script with old-tag argument handling                                                                                  | 1      | 1      |~~ done — git-archive+go-doc steps in the Release Ritual
~~| 20 | Decide whether Deferred Register items graduate into TODO_LIST when triggered (doc the promotion path)                                                           | 1      | 1      |~~ done — Deferred Register lives in AGENTS with trigger conditions
~~| 21 | cqrs README: document Status() as a slice (godoc examples now cover it; README doesn't)                                                                          | 1      | 1      |~~ NOT-DO — superseded by cqrs v0.5.0 (Bundle/Status surface replaced)
~~| 22 | Realtime+health integration example: drain-aware SSE shutdown (health module's DrainHooks story)                                                                 | 2      | 2      |~~ NOT-DO — example idea; DrainHooks story documented in health README
~~| 23 | LagPerProjection → otel gauge wiring example (checkpoint.lag instrument exists; show alert math)                                                                 | 1      | 1      |~~ NOT-DO — no demand
~~| 24 | SQLite busy_timeout/WAL tuning doc for contended event stores                                                                                                    | 2      | 1      |~~ NOT-DO — no demand
~~| 25 | Fuzz or property test for staleness guards (budget boundary: exactly-at-threshold)                                                                               | 2      | 2      |~~ NOT-DO — no demand
~~| 26 | Multi-recorder coordination ADR (HTTP + projections + health triggers on ONE fr.Recorder) — the README claims it, an ADR would own it                            | 2      | 1      |~~ NOT-DO — README claims + one-recorder gotcha suffice
~~| 27 | Trigger-context documentation table (Kind values across cqrs + appkit middleware)                                                                                | 1      | 1      |~~ done — trigger contexts documented in module docs
~~| 28 | ReplayResult caller-contract Example (delete-after-replay) — started in godoc prose, make it an Example                                                          | 1      | 1      |~~ NOT-DO — prose suffices
~~| 29 | Card: check `example/` (root) still reflects v0.4.0 hooks; it built green but content may predate DrainHooks                                                     | 1      | 1      |~~ done — example compiles (verified 2026-08-17/09-04)
