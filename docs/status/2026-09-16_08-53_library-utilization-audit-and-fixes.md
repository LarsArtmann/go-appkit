...
| # | Work                       | Done                                                                                          | Missing                                                                                                                                                                           |
| - | -------------------------- | --------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~1~~ | ~~Audit fix execution~~ done — composition routed to TODO_LIST P2; items 5-12 routed across TODO_LIST P3 + ROADMAP (2026-09-16 harvest) | ~~4 of 12 Pareto items executed (dashboard bump, drain fix, error-family bump ×6, frhealth pin)~~ | ~~Items 2 (Server composition), 5-12 still open — see (c)~~ |
| ~~2~~ | ~~Verification depth~~ done — origin-currency proof routed to TODO_LIST P3; the composition recommendation is labeled spike-first in the TODO P2 item | ~~Bumps proven by tag diffs + tests~~ | ~~"All latest" claims verified against LOCAL clones only, not origin (`git ls-remote`); no compile spike for the Server-composition recommendation~~ |
| ~~3~~ | ~~AGENTS.md accuracy~~ done — re-reviewed end-to-end 2026-09-16 (second docs-health pass) | ~~My corrections landed and merged~~ | ~~Concurrent session edited the file twice mid-session (cqrs v0.5.0 docs, ghost-release P1 note, CI section); final state not re-reviewed end-to-end by me~~ |
| ~~4~~ | ~~Health example E2E~~ done — routed to TODO_LIST P3 (health example live E2E re-run) | ~~Unit + race green after drain change~~ | ~~Live lockstep-drain E2E (the module's own documented claim) not re-run~~ |
| ~~5~~ | ~~Audit report freshness~~ done — 2026-09-16 — fixed findings annotated in the HTML report; module count corrected inline | ~~Point-in-time by design~~ | ~~No ANNOTATE pass marking which findings this session then fixed; its "9 modules" count was stale on arrival~~ |
| ~~6~~ | ~~Lint hygiene on new code~~ done — closed 2026-09-16 — health + frhealth lint 0 issues; the sleep is inside a deadline-bounded poll loop (the house waitForRunning pattern) | ~~`go vet` clean~~ | ~~golangci-lint not run on health/flightrecorderhealth; new test's `time.Sleep` poll likely trips the repo's no-sleep testing rule (and possibly wsl_v5)~~ |
| ~~7~~ | ~~Release state of the fixes~~ done — routed to TODO_LIST P3 (version cuts + missing error-family CHANGELOG lines) | ~~CHANGELOGs written; daemon committed the code~~ | ~~No version cuts; CHANGELOG entries for the error-family bump exist only in health + flightrecorderhealth, not cqrs/errorpages/root; daemon's heuristic commits bury the rationale~~ |

## c) NOT STARTED

1. ~~**httputil.Server composition spike** — the audit's #2 opportunity (15 pts): prove `ServerConfig` covers `ServiceConfig` (NoTimeout→zero-field mapping, drain ordering), then refactor. Unlocks item 2.~~ done (routed to TODO_LIST P2 (composition item, spike-first, USER GATE on API posture))
2. ~~**Core TLS option** via `Server.StartTLS` — currently in the deferred register; becomes nearly free after item 1.~~ done (owned by AGENTS Deferred Register; unlocked by the TODO P2 composition item (StartTLS))
3. ~~**Dashboard hardening passthrough** — `WithDashboard` presets for `WithShutdownDrain`, `WithNonce`+`RecommendedCSP`, `WithRateLimit` (audit #7).~~ done (routed to TODO_LIST P3 (dashboard hardening passthrough))
4. ~~**realtime drop visibility** — `WithOnDrop` counter + log line + `Hub.Health` field (audit #6).~~ done (covered by TODO_LIST P3 W5 C1 (dedupe noted there))
5. ~~**Flightrecorder ops preset** — `WithRecorderOptions` passthrough; production preset with `WithSnapshotDir`/`WithMaxSnapshots`/`WithCompression`/`WithMetrics` (audit #4); wire `MetricsHook` into the otel module's meter.~~ done (routed to TODO_LIST P3 (flightrecorder ops preset + metrics bridge))
6. ~~**go-health payload richness** — `WithVersion`/`WithBootTime`/`WithInstanceID` wired from buildinfo = battery item F5 (audit #5); document `WithGETOnly`+`WithLiveThrottle` public-exposure preset; evaluate `aggregate` package for multi-instance views.~~ done (folded into TODO_LIST P2 W1 F5 BuildInfo (health-module wiring))
7. ~~**errorfamilytest adoption** — replace hand-rolled family assertions in cqrs/health/errorpages tests with `errorfamilytest.Assert*` (mechanical; I over-deferred this).~~ done (routed to TODO_LIST P3 (error-classification sweep))
8. ~~**Classify the 3 bare-sentinel error sites** — flightrecorder `errHTTPStatus` (middleware.go:120-131), otel setup sentinels (setup.go:122-155), cqrs drain (eventservice.go:729).~~ done (routed to TODO_LIST P3 (error-classification sweep))
9. ~~**Upstream ask to go-sse** — dedup-aware `ReplayFiltered` variant (appkit's handler.go:180-261 re-implements replay for live-dedup; verified `ReplayFiltered` has none).~~ done (routed to TODO_LIST P3 (go-sse dedup-aware ReplayFiltered ask))
10. ~~**httputil v1.2 watch** — when `ff44c5f` (Pattern propagation) tags: bump, re-verify otel span names through `OuterMiddlewares`, close TODO P2; note the Compression absent-encoding default change (no appkit impact — default stack doesn't enable Compression).~~ done (owned by TODO_LIST P2 OTEL release train)
11. ~~**HARVEST** — route this report's (f) list into TODO_LIST.md/ROADMAP.md (waiting for your go, per "THEN WAIT").~~ done (done 2026-09-16 — second docs-health pass routed (f) into TODO_LIST/ROADMAP)
12. ~~**ANNOTATE** the audit report (fixed-findings markers) and reconcile its 9-vs-10 module count.~~ done (done 2026-09-16 (this pass))
13. ~~**gopls debt** — `infertypeargs` ×7 (`cqrs/commands.go:60,70`, `commands_test.go:73,90,187,245`, `realtime/hub.go:54`), unused `expectError`/`freePort` (`testhelpers_test.go:77,85`). Pre-existing; visible all session; untouched.~~ done (done 2026-09-16 — expectError/freePort deleted (0 usages verified); the infertypeargs sites already carry explicit type args)
14. ~~**Proxy/origin currency proof** — `git ls-remote --tags` on the 11 family repos vs pins.~~ done (routed to TODO_LIST P3 (dependency-currency proof))
15. ~~**docs-mod ghost release (P1)** — module path `.../docs` vs directory `docs-mod/` makes the v0.2.0 tag unfetchable from the proxy (surfaced by the concurrent session's notes; not my work, but it's the highest-priority open release bug in the repo).~~ done (owned by TODO_LIST P1)
