...
| Item                               | Impact   | Notes                                                                                                                                               |
| ---------------------------------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
~~| **Tag v1.0.0**                     | Critical | Core module is ready. No git tag exists.                                                                                                            |~~ resolved
~~| **Tag cqrs v0.1.0**                | Medium   | CQRS module is ready. No git tag exists.                                                                                                            |~~ resolved
~~| **Tag docs v0.1.0**                | Low      | Docs module is ready. No git tag exists.                                                                                                            |~~ resolved
~~| **CQRS README section**            | Medium   | No usage documentation for the cqrs sub-module                                                                                                      |~~ resolved
~~| **Docs README section**            | Low      | No usage documentation for the docs sub-module                                                                                                      |~~ resolved
~~| **docs-mod → docs rename**         | High     | Directory is `docs-mod/` but module path is `github.com/larsartmann/go-appkit/docs`. This is confusing. The `docs/` directory holds Markdown files. |~~ resolved
~~| **CQRS + Service.Run integration** | Medium   | No `ServiceConfig.CQRS` field or `Service.UseCQRS()` method                                                                                         |~~ resolved
~~| **flake.nix**                      | Low      | AGENTS.md says "no flake.nix, use standard Go tooling"                                                                                              |~~ resolved
~~| **CHANGELOG.md**                   | Low      | No changelog for the rewrite                                                                                                                        |~~ resolved
~~| **golangci-lint for sub-modules**  | Medium   | depguard config doesn't cover cqrs/docs sub-module imports                                                                                          |~~ resolved
...
| ---------------------------------------------------------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
~~| **`docs-mod/` directory name vs `docs/` module path**      | **HIGH**  | The module is `github.com/larsartmann/go-appkit/docs` but the directory is `docs-mod/`. Go module tooling resolves it via go.work replace directive, but ANY external consumer importing `github.com/larsartmann/go-appkit/docs` will get a 404 from the repo because the directory doesn't match the module path. **This must be fixed before tagging.** |~~ resolved
~~| **README mentions `os` import but example doesn't use it** | Low       | README quick-start code shows `errorfamily.HandleError(err)` but doesn't import `os`. Copy-paste will fail.                                                                                                                                                                                                                                               |~~ resolved
~~| **LSP stale typecheck warnings**                           | Cosmetic  | 5 stale "other declaration of defaultReadTimeout" warnings from LSP cache. `go build` and `go vet` pass clean. LSP hasn't refreshed after server.go deletion.                                                                                                                                                                                             |~~ resolved
~~| **Test runtime 5-6s**                                      | Low       | Default DrainDelay=5s makes some tests slow. Non-drain tests use DrainDelay:0 but some still wait on Close().                                                                                                                                                                                                                                             |~~ resolved
~~| **`httpspec_test.go` uses `init()` hack**                  | Low       | The `init()` function suppresses unused import for httptest. Should be removed — httptest is used by httpspec internally, not by our test.                                                                                                                                                                                                                |~~ resolved
~~| **No `ServiceConfig.CQRS` integration**                    | Medium    | The plan called for `Service.Run()` to call `EventService.Shutdown()` during graceful drain. This is NOT implemented. Consumer must manually coordinate.                                                                                                                                                                                                  |~~ resolved
~~| **Uncommitted go.sum changes**                             | **FIXED** | go.work pulled newer indirect dep versions. Committed at 9356d1d.                                                                                                                                                                                                                                                                                         |~~ resolved
