...
> **2026-09-16 docs-health:** every tier item below maps to a C-task in the
> Comprehensive Plan — the C-rows carry the verdicts (struck = done, owned by
> a living doc, or explicitly not taken). The Fine-Grained F-tasks inherit
> their parent C-task's verdict; only C23's example remainder moved to
> ROADMAP raw ideas and C1/C6/C11/C14 remain gated in TODO_LIST.
...
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- | -------------------------------- | ------ | ------ | ----- | -------------------- |
| ~~C1~~  | ~~License posture decision + wave #2 membership definition (proprietary vs MIT-family; which tags ship LICENSE)~~ done — license owned by TODO_LIST P2 (USER GATE); wave membership executed | ~~1%~~ | ~~P2-license (UG)~~ | ~~5~~ | ~~2~~ | ~~5~~ | ~~—~~ |
| ~~C2~~  | ~~otel/v0.1.0 release prep: drop example `replace ../`, require published core, `GOWORK=off` hermetic verify, annotated tag~~ done — otel/v0.1.0 shipped 2026-09-04 (wave 2) | ~~1%~~ | ~~P1-otel~~ | ~~5~~ | ~~3~~ | ~~5~~ | ~~C1~~ |
| ~~C3~~  | ~~health/v0.1.0 release prep: same ritual for the new health module~~ done — health/v0.1.0 shipped 2026-09-04 | ~~1%~~ | ~~P1-health~~ | ~~5~~ | ~~3~~ | ~~5~~ | ~~C1~~ |
| ~~C4~~  | ~~flightrecorderhealth/v0.1.1 release prep: go-health v0.1.1 bump, jsonv2-requirement release note, verify, tag~~ done — flightrecorderhealth/v0.1.1 shipped 2026-09-04 | ~~1%~~ | ~~P1-frh~~ | ~~4~~ | ~~2~~ | ~~4~~ | ~~C1~~ |
| ~~C5~~  | ~~Wave #2 push + per-tag fresh-consumer proxy tests~~ done — wave 2 pushed; proxy smoke PASS (18-57 a2) | ~~1%~~ | ~~P1-postpush~~ | ~~5~~ | ~~2~~ | ~~5~~ | ~~C2,C3,C4~~ |
| ~~C6~~  | ~~pkg.go.dev render verification: every module page exists, license classified, godoc visible; record before/after screenshots in status doc~~ done — re-crawl owned by TODO_LIST P1 (license gate + docs ghost) | ~~1%~~ | ~~P1-postpush+P2-license~~ | ~~5~~ | ~~2~~ | ~~5~~ | ~~C5~~ |
| ~~C7~~  | ~~cqrs v0.4.0: CHANGELOG date, tag, push, fresh-consumer verify of the breaking FlightRecorder migration~~ done — cqrs/v0.4.0 2026-09-04; superseded by v0.5.0 (2026-09-07, system engine) | ~~4%~~ | ~~session-status #1~~ | ~~4~~ | ~~3~~ | ~~4~~ | ~~— (parallel to wave)~~ |
| ~~C8~~  | ~~API-break check wired into release checklist (goapidiff or go-doc snapshot diff), documented in AGENTS.md release ritual~~ done — Release Ritual step 1, proven on v0.3.0→v0.4.0 | ~~4%~~ | ~~P2-apibreak~~ | ~~4~~ | ~~2~~ | ~~4~~ | ~~—~~ |
| ~~C9~~  | ~~README build-from-source section: GOEXPERIMENT=jsonv2 module table (root, cqrs, realtime, otel-no, docs, errorpages, flightrecorder, flightrecorderhealth-now-yes, integration-no)~~ done — 2026-09-04 (18-57 a7 — jsonv2 default-on made the matrix moot) | ~~4%~~ | ~~P2-readme~~ | ~~3~~ | ~~2~~ | ~~4~~ | ~~C4 (frh jsonv2 flip)~~ |
| ~~C10~~ | ~~DrainDelay sweep: convert satellite suites to NoDrainDelay, measure wall-time before/after~~ done — 2026-09-04 (18-57 C10) | ~~4%~~ | ~~P2-draindelay~~ | ~~3~~ | ~~2~~ | ~~3~~ | ~~—~~ |
| ~~C11~~ | ~~Logging posture: pick option (level config / sampling / consumer-logger), implement, benchstat before/after vs spike numbers~~ done — owned by TODO_LIST P2 (data recorded: +30µs emitting vs +0.8µs suppressed) | ~~4%~~ | ~~P2-logging (UG on option)~~ | ~~4~~ | ~~4~~ | ~~4~~ | ~~—~~ |
| ~~C12~~ | ~~core release carrying DrainHooks: verify health module's dep direction, tag (patch/minor decision via API-break check C8)~~ done — core v0.4.0 (2026-09-04) | ~~4%~~ | ~~P1-health-a~~ | ~~4~~ | ~~3~~ | ~~4~~ | ~~C1, C8~~ |
| ~~C13~~ | ~~Release-state single source: reconcile AGENTS.md ↔ TODO_LIST.md; verify+close stale "commit the OTEL work" item (check `aaa2427` coverage); docs-health ANNOTATE pass on old status reports~~ done — AGENTS Release State current; 2026-09-16 docs-health passes; single-owner decision in TODO_LIST P3 | ~~20%~~ | ~~P1-otel-stale + session #3~~ | ~~2~~ | ~~2~~ | ~~3~~ | ~~—~~ |
| ~~C14~~ | ~~dprint config fix: `--allow-no-files` (or skip-on-empty) so CHANGELOG-only commits stop exiting 14~~ done — owned by TODO_LIST P3 + AGENTS Deferred Register (upstream fix) | ~~20%~~ | ~~P3-dprint~~ | ~~2~~ | ~~1~~ | ~~2~~ | ~~—~~ |
| ~~C15~~ | ~~go-structure-linter: configure root-package acceptance for core; get to zero standing errors~~ done — 2026-09-04 (18-57 a11); re-verified 0 findings 2026-09-16 | ~~20%~~ | ~~P3-linter~~ | ~~2~~ | ~~1~~ | ~~2~~ | ~~—~~ |
| ~~C16~~ | ~~otel middleware benchmark: no-op vs configured, benchstat, numbers into otel README~~ done — otel/benchmark_test.go + README (re-baselined n=10, 2026-09-16) | ~~20%~~ | ~~P3-otelbench~~ | ~~2~~ | ~~2~~ | ~~2~~ | ~~—~~ |
| ~~C17~~ | ~~Upstream cqrs-lite otel ForceFlush: reproduce in minimal probe, verify-before-filing, file issue/PR (UG: issue vs PR)~~ done — resolved 2026-09-15 — verified FIXED upstream, no filing needed | ~~20%~~ | ~~P2-upstream~~ | ~~3~~ | ~~2~~ | ~~3~~ | ~~—~~ |
| ~~C18~~ | ~~design-decisions.md: fix lychee 404 + wrap MD013 long lines~~ done — 404 inlined 2026-09-04 (18-57 a16) | ~~20%~~ | ~~P2-docs~~ | ~~1~~ | ~~1~~ | ~~1~~ | ~~—~~ |
| ~~C19~~ | ~~Toolchain bump 1.26.5 → 1.26.6+ across modules when nixpkgs carries it (GOTOOLCHAIN=local means nixpkgs gates; re-run govulncheck)~~ done — for 1.26.7 (all modules); next bump owned by TODO_LIST P2 | ~~20%~~ | ~~P2-toolchain (gated)~~ | ~~2~~ | ~~1~~ | ~~2~~ | ~~external~~ |
| ~~C20~~ | ~~cqrs-lint 4.8.1 → `~/go/bin` + document `.` arg form in cqrs README gotchas~~ done — 2026-09-04 (18-57 a14) | ~~20%~~ | ~~P3-cqrslint~~ | ~~1~~ | ~~1~~ | ~~1~~ | ~~—~~ |
| ~~C21~~ | ~~cqrs test hardening: trigger-returns-false test + HostOptions-cannot-override-derived-wiring test~~ done — 18-57 a12 (both tests) | ~~20%~~ | ~~session #18~~ | ~~2~~ | ~~2~~ | ~~2~~ | ~~—~~ |
| ~~C22~~ | ~~core v1.0.0 exit criteria draft (fold OuterMiddlewares/ShutdownHooks; link framework-architecture.md ideas)~~ done — docs/planning/core-v1-exit-criteria.md | ~~20%~~ | ~~P3-v1criteria~~ | ~~2~~ | ~~2~~ | ~~2~~ | ~~—~~ |
| ~~C23~~ | ~~cqrs examples: shared fr.Recorder across appkit middleware + projections; OTel→Prometheus pairing snippet; DLQ admin + replay-contract examples~~ done — partially — 2 godoc Examples landed (wiring + replay); shared-fr-recorder demo is a ROADMAP raw idea | ~~rest~~ | ~~session #20~~ | ~~2~~ | ~~2~~ | ~~2~~ | ~~C7~~ |
| ~~C24~~ | ~~exhaustruct → exhaustruct_v5 migration in all module .golangci.yml files (kill deprecation warning)~~ done — all 10 configs (18-57 a10) | ~~rest~~ | ~~session #21~~ | ~~1~~ | ~~2~~ | ~~1~~ | ~~—~~ |
| ~~C25~~ | ~~Adoption instrumentation ritual: scorecard after each wrapper feature + cookbook drift guard (documented as checklist lines in AGENTS.md)~~ done — AGENTS Adoption & Drift Rituals | ~~rest~~ | ~~session #24~~ | ~~1~~ | ~~1~~ | ~~1~~ | ~~—~~ |
| ~~C26~~ | ~~Deferred-by-design register (DO NOT BUILD): encryption/signing/idempotency/scheduling opt-ins; cordis bridge; PapDashboard appkit-side code; core TLS until PapDashboard demands it — one checklist block with trigger conditions~~ done — AGENTS Deferred Register (trigger conditions only) | ~~rest~~ | ~~P3-cordis, P3-papdash, P3-optins~~ | ~~1~~ | ~~1~~ | ~~2~~ | ~~—~~ |
