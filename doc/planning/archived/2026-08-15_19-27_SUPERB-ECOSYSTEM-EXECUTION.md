...
| ~~     | 3  | M03  | Migrate cqrs module to go-cqrs-lite v4 (paths, API, jsonv2, tests)                                    | 2      | 5      | 3        | 15   | 100 min    |
| ~~     | 4  | M04  | Migrate docs module to catalog/v4 + docserver v4                                                      | 2      | 4      | 2        | 16   | 45 min     |
| ~~     | 5  | M05  | `EventConfig.Logger` → `projectionhost.WithLogger` + test                                             | 2      | 4      | 1        | 20   | 30 min     |
| ~~     | 6  | M06  | `EventConfig.DLQ` (SQLite dead-letter store) + replay/purge accessors + tests                         | 3      | 4      | 2        | 16   | 60 min     |
| ~~     | 7  | M07  | `EventConfig.FlightRecorder` → `WithFlightRecorder` + test + cross-link docs                          | 3      | 3      | 1        | 15   | 30 min     |
| ~~     | 8  | M08  | Projection readiness: `Status()`/`LagPerProjection()` → `/health/ready` integration + tests           | 3      | 4      | 2        | 16   | 60 min     |
| ~~     | 9  | M09  | New opt-in `errorpages` module wrapping templ-components/errorpage (Mount, JSON mode, tests, example) | 3      | 4      | 3        | 12   | 90 min     |
| ~~     | 10 | M10  | Metrics path: otel/prometheus accessors for the event store                                           | 4      | 3      | 3        | 9    | 60 min     |
| ~~     | 11 | M11  | Journal→SSE bridge: cqrs `event.Journal` → `sse.EventStore` adapter in its own module                 | 4      | 4      | 3        | 12   | 100 min    |
| ~~     | 12 | M12  | Context hygiene: realtime/handler.go, service.go, docs_test.go request contexts                       | 4      | 2      | 1        | 10   | 30 min     |
| ~~     | 13 | M13  | Docs refresh: README, FEATURES, CHANGELOGs, AGENTS.md (v4, options, jsonv2)                           | 4      | 3      | 1        | 15   | 45 min     |
| ~~     | 14 | M14  | Upstream courtesy fix: FamilyOrchestration in templ-components/errorpage status map (verify → PR)     | 4      | 2      | 2        | 8    | 45 min     |
| ~~     | 15 | M15  | Releases: verify all modules, cut CHANGELOGs, tag cqrs v0.2.0 + docs v0.2.0 + errorpages v0.1.0       | 4      | 3      | 2        | 12   | 45 min     |
| ~~     | 16 | M16  | ADR-002: EventService future — sqlite-first vs stack-generic vs `system` package adoption             | 4      | 3      | 3        | 9    | 45 min     |
| ~~     | 17 | M17  | cqrs README cookbook: scenario DSL, testutil, cqrs-lint leverage                                      | 4      | 2      | 1        | 10   | 30 min     |
| ~~     | 18 | M18  | cqrs-htmx prototype spike: appkit.Service behind `setup.Run` (conditional on ADR-001)                 | 4      | 4      | 4        | 8    | 100 min    |
