...
1. ~~**License posture decision execution** (USER GATE since August) — blocks pkg.go.dev godoc for every module; the README now truthfully documents the proprietary consequence.~~ done (DECIDED 2026-09-16 — stays proprietary (user ruling); consequence documented in the README)
2. ~~**The docs-module ghost fix itself** (directory/module-path reconciliation + `docs/v0.3.0` re-tag + fresh-consumer proxy test) — documented as P1 with options; needs the user's path choice and the push gate.~~ done (FIXED 2026-09-16 — path A executed; docs/v0.3.0 tagged, pushed, proxy-proven)
3. ~~**pkg.go.dev re-crawl verification** for all released tags (gated on license + the docs re-tag).~~ done (VERIFIED 2026-09-17 — every module page renders (docs v0.3.0, health v0.1.1, security v0.1.0, core v0.5.1))
4. ~~**The otel regression release train** — the parallel session fixed httputil upstream (ships v1.2); the integration-module regression test (span name + `http.route` through the full default stack) and the otel re-tag remain.~~ done (SHIPPED 2026-09-16 — httputil v1.2.0 + otel v0.1.1; integration/otel_pattern_test.go pins it against PUBLISHED tags)
5. ~~**benchstat re-baseline** of the otel benchmark at the recorded 3×1s-median protocol (TODO_LIST P3).~~ done (BASELINE RECORDED 2026-09-16 — n=10 mean±sd in the otel README; real benchstat stays a candidate)
