...
- Library consumed by Go applications (reference consumer: cqrs-htmx `setup` — `RunWithAppkit` stable there since 2026-09-07 on core v0.4.0, default-flip (b)-(f) open in THEIR TODO_LIST P3, v0.5.x bump + Metrics/Version threading planned in their P1 setup-gap bundle; setup is NEVER a dependency of appkit — direction is setup → core only). Source in repository root; example in `example/main.go`.
