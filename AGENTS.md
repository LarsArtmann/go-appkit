...
- Eleven Go modules in one repo, independently versioned (ten released + the unreleased `integration` test module):
  ...
  - **flightrecorderhealth** (`/flightrecorderhealth`) — package `flightrecorderhealth`, bridges go-flightrecorder with go-health: dashboard visibility + auto-capture on health failures. v0.1.2 (2026-09-16; go-health v0.1.3).
  - **health** (`/health`) — package `health` (alias `appkithealth`), bridges go-health probes + go-health-dashboard real-time UI into appkit services. v0.1.1 (2026-09-16 two-phase drain; DashboardHardenedPreset + godoc examples landed after, unreleased). See the Health Module section below.
  ...
  - **integration** (`/integration`) — cross-module + cross-repo E2E composition tests, never released. Pins PUBLISHED tags (core v0.5.1, realtime v0.1.1, errorpages v0.1.0, otel v0.1.1, cqrs-htmx v4.9.0/transport) so it always tests what consumers resolve; carries the composition-contract suite (metrics/version/testkit compose, drain-window ordering — `svc.Addr()` is nil inside DrainHooks, pinned 2026-09-17); does NOT require `GOEXPERIMENT=jsonv2` (transport is lean; the otel pin does not change this). Added 2026-09-04. See the Integration Module section below.
- Library consumed by Go applications (reference consumer: cqrs-htmx `setup` — `RunWithAppkit` stable there since 2026-09-07; their setup pins core v0.5.0 since 2026-09-17 (their M3 bump; was v0.4.0), default-flip (b)-(f) and the Metrics/Version threading (M4) open in THEIR TODO_LIST; setup is NEVER a dependency of appkit — direction is setup → core only). Source in repository root; example in `example/main.go`.
...
- Every module tag is ON ORIGIN through **core v0.5.1** (2026-09-17): core v0.5.1 (doc-only; v0.5.0 the same day), security v0.1.0 / realtime v0.1.1 / health v0.1.1 / flightrecorderhealth v0.1.2 / docs v0.3.0 (ghost un-ghosted) / otel v0.1.1 + httputil v1.2.0 (2026-09-16), cqrs v0.5.0 (2026-09-07), flightrecorder v0.1.0, errorpages v0.1.0. Wave history and per-version deltas live in the module CHANGELOGs; the pending-work queue lives in TODO_LIST.
...
- appkit does NOT delegate to `httputil.Server` — and the composition is now BLOCKED on an upstream API, not pending: the spike (`doc/planning/2026-09-16_composition-spike-verdict.md`) proved httputil's `Start()` binds internally with ASYNC bind errors vs appkit's synchronous `listen_failed` contract (public `Start() (<-chan error, error)`), so a byte-identical swap is impossible until httputil exposes listener injection (`NewServerListener`). The Core TLS option is gated on the same ask. The 6/7-field config mapping is clean; only the lifecycle seam is missing.
...
| Cordis bridge module                                                                  | cordis tags go/v0.1.0 AND a consumer states the reactive-composition requirement AND core v1.0.0 exit criteria shipped (`doc/planning/core-v1-exit-criteria.md`)                                                                                                                                                                                                                                       |
...
| httputil `docs/integrations/huma.md` 404 on GitHub                                    | File exists unpushed in the httputil repo — push it, then optionally re-link from `doc/planning/design-decisions.md` Decision 7 (currently inlined instead)                                                                                                                                                                                                                                            |
